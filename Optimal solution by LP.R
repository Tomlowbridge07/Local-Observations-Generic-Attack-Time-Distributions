...
{
 return(CostVec[Node]*LambdaVec[Node]*(integrate(AttackCDFVec[[Node]],StateVector[Node]-1,StateVector[Node])$value))
}
...
#Moving instaneously with local observations
CostOfActionOnNode<-function(Node,StateVector,NodeMovedTo,n,CostVec,LambdaVec,AttackCDFVec,AttackPDFVec,BVec,bVec)
{
  if(Node==NodeMovedTo)
  {
  ...
  {
    return(CostVec[Node]*LambdaVec[Node]*(integrate(AttackCDFVec[[Node]],StateVector[Node]-1,StateVector[Node])$value) 
           + CostVec[Node]*StateVector[n+Node]*(integrate(AttackPDFVec[[Node]],StateVector[Node]-1,StateVector[Node])$value))
  }
}

#Total action cost
CostOfAction<-function(StateVector,NodeMovedTo,n,CostVec,LambdaVec,AttackCDFVec,AttackPDFVec,BVec,bVec)
{
  Sum=0
  for(j in 1:n)
  {

    Sum=Sum+CostOfActionOnNode(j,StateVector,NodeMovedTo,n,CostVec,LambdaVec,AttackCDFVec,AttackPDFVec,BVec,bVec)
...
CostToNeglect<-function(StateVector,n,CostVec,LambdaVec,AttackCDFVec,AttackPDFVec,BVec,bVec)
{
  
  #We can use prior formula but with no node every visited
  return(CostOfAction(StateVector,-1,n,CostVec,LambdaVec,AttackCDFVec,AttackPDFVec,BVec,bVec))
}

#instaneous moving and local observations
SeperatedCostOfActionOnNode<-function(Node,StateVector,NodeMovedTo,n,CostVec,LambdaVec,AttackCDFVec,AttackPDFVec,BVec,bVec)
{
  ...
  {
    return(list(CostDueToArrivals=CostVec[Node]*LambdaVec[Node]*(integrate(AttackCDFVec[[Node]],StateVector[Node]-1,StateVector[Node])), 
           CostDueToObserved=CostVec[Node]*StateVector[n+Node]*integrate(AttackPDFVec[[Node]],StateVector[Node]-1,StateVector[Node])$value))
  }
  
}
 
SeperatedCostOfAction<-function(StateVector,NodeMovedTo,n,CostVec,LambdaVec,AttackCDFVec,AttackPDFVec,BVec,bVec)
{
  SumDueToArrivals=0
  SumDueToObserved=0
  for(j in 1:n)
  {
    SumDueToArrivals=SumDueToArrivals+SeperatedCostOfActionOnNode(j,StateVector,NodeMovedTo,n,CostVec,LambdaVec,AttackCDFVec,AttackPDFVec,BVec,bVec)$CostDueToArrivals
    SumDueToObserved=SumDueToObserved+SeperatedCostOfActionOnNode(j,StateVector,NodeMovedTo,n,CostVec,LambdaVec,AttackCDFVec,AttackPDFVec,BVec,bVec)$CostDueToObserved
  }
...
ActiveConstraints<-function(AdjMatrix,n,CostVec,LambdaVec,AttackCDFVec,AttackPDFVec,BVec,bVec)
{
...
ActiveConstraintsExperimental<-function(AdjMatrix,n,CostVec,LambdaVec,AttackCDFVec,AttackPDFVec,BVec,bVec)
{
  #Retrive the solution to the LP, Solve for the solution
...
FindOptimalEquilibriumValuesByDual<-function(AdjMatrix,n,CostVec,LambdaVec,AttackCDFVec,AttackPDFVec,BVec,bVec,AlphaVec=NULL)
{
  ...
  OptimalSol=Solved$solution
  
  return(list(OptimalEquilibrium=OptimalSol))
